...
# just adding to the file with most of the filenames
...
do
	echo $SPID
	FILEID=`jamo info all spid $SPID | grep -i final.contigs.fasta | awk '{ print $4 }'`	
	FILENAME=`jamo info all spid $SPID | grep -i final.contigs.fasta | awk '{ print $2 }'`	
