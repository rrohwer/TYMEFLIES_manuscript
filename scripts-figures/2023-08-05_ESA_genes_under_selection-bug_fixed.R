...
# for acI-A ----
pdf(file = "figures/2023-08-05_ESA_plots-bug-fixed/gene_selection_acI-A_seasons.pdf", width = 6.5, height = 3)
...
# for acI-C ----
pdf(file = "figures/2023-08-05_ESA_plots-bug-fixed/gene_selection_acI-C_seasons.pdf", width = 6.5, height = 3)
bar.spots <- barplot(height = season.sum$Npos.per.sample, beside = T, plot = F, space = .75)
x.lim <- c(min(bar.spots), max(bar.spots))
y.lim <- c(-max(season.sum$Nneg.per.sample, na.rm = T), max(season.sum$Npos.per.sample, na.rm = T))
par(mar = c(2.5,6,2,1))
...
# for acI-B ----
pdf(file = "figures/2023-08-05_ESA_plots-bug-fixed/gene_selection_acI-B_long-term.pdf", width = 6.5, height = 3)
...
abline(h = 0)
text(x = bar.spots[seq.int(1,20,2)], y = -3, labels = year.sum$year[seq.int(1,20,2)], cex = 1, xpd = NA, srt = 90)
y.lim
ticks <- axis(side = 2, line = 0, labels = F, tck = -.02, at = seq.int(-2,8,2), xpd = T)
axis(side = 2, line = -.5, at = ticks, labels = abs(ticks), lwd = 0, las = 2, cex.axis = 1, xpd = T)
mtext(text = "Genes Under Selection (ave / sample)", side = 2, line = 4, cex = 1, at = 2.8)
mtext(text = "Positive", side = 2, line = 2.3, at = .25, cex = 1, adj = 0)
mtext(text = "Purifying", side = 2, line = 2.3, at = -.25, cex = 1, adj = 1)
x.lim
arrows(x0 = -2, y0 = .25, x1 = -2, y1 = 3, xpd = T, length = .075)
arrows(x0 = -2, y0 = -.25, x1 = -2, y1 = -3.3, xpd = T, length = .075)
dev.off()
# ----