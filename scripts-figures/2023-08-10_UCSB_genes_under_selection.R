...
# # ----
# for acI-B ----
pdf(file = "figures/2023-08-10_UCSB_plots/gene_selection_acI-B_long-term.pdf", width = 3.1, height = 1.36)
...
barplot(height = -year.sum$Nneg.per.sample, beside = T, names.arg = NULL, axes = F, ann = F, axisnames = F, col = adjustcolor("#a9bfdb", alpha.f = 1), add = T, border = NA)
abline(h = 0)
axis(side = 1, at = c(min(bar.spots), max(bar.spots)), lwd = 1, lwd.ticks = 0, labels = F, line = 0, xpd = NA)
axis(side = 1, at = bar.spots[c(1,13,20)], labels = F, tck = -.05, lwd = 0, lwd.ticks = 1)
axis(side = 1, at = bar.spots[c(1)], labels = year(x.ticks)[1], lwd = 0, las = 1, line = -.75, hadj = .25)
axis(side = 1, at = bar.spots[c(13)], labels = year(x.ticks)[2], lwd = 0, las = 1, line = -.75, hadj = .5)
axis(side = 1, at = bar.spots[c(20)], labels = year(x.ticks)[3], lwd = 0, las = 1, line = -.75, hadj = .75)

ticks <- axis(side = 2, line = 0, labels = F, tck = -.05, at = c(-40,-20,0,10), xpd = T)
axis(side = 2, line = -.5, at = ticks, labels = abs(ticks), lwd = 0, las = 2, cex.axis = 1, xpd = T)
mtext(text = "Genes Under\nSelection", side = 2, line = 1.8, cex = 1)
