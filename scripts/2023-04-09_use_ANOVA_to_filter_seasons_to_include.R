...
output.data.folder <- "data/2023-04-09_genome_abund-div_correlations"

breadth.cutoff <- .7
season.cutoff <- 2 # must be in this many seasons to be included in seasonal analysis
...
seasonal <- rel.abund[order(genome, year, season), .(nuc.div = mean(nucl_diversity), abund.perc = mean(abund.perc)), by = .(genome, year, season)]

# be in at least 2 seasons at least 5 times to include
keep <- seasonal[ , .N, by = .(season, genome)]
keep[ ,N := N >= season.cutoff.2]
keep <- keep[ , .(N = sum(N)), by = genome]
keep <- keep[N >= season.cutoff]
seasonal <- merge(x = keep[ , .(genome)], y = seasonal, by = "genome", all.x = T, all.y = F)
length(unique(seasonal$genome)) # 1261

# have a seasonal pattern to be included in the analysis- at least one season's nuc div different from the other season's
seasonal[ ,anova.pvalue := 999]
for (g in unique(seasonal$genome)){
  my.anova <- aov(nuc.div ~ season, data = seasonal[genome == g, ]) |>
    summary()
  seasonal[genome == g, anova.pvalue := my.anova[[1]]$`Pr(>F)`[1]]
}
seasonal <- seasonal[anova.pvalue < .05]
length(unique(seasonal$genome)) # 485
...
annual <- keep.annual[order(genome, year, season), .(nuc.div = mean(nucl_diversity), abund.perc = mean(abund.perc)), by = .(genome, year)]
length(unique(annual$genome))

# have an annual pattern to be included in the analysis- at least one year's nuc div different from other years
annual[ ,anova.pvalue := 999]
for (g in unique(annual$genome)){
  my.anova <- aov(nuc.div ~ year, data = annual[genome == g, ]) |>
    summary()
  annual[genome == g, anova.pvalue := my.anova[[1]]$`Pr(>F)`[1]]
}
hist(annual$anova.pvalue)
annual <- annual[anova.pvalue < .05]
length(unique(annual$genome)) # 210

# ---- make into matrices ----

d.seasonal <- dcast(data = seasonal, formula = genome ~ season + year, value.var = "nuc.div")
d.seasonal <- as.matrix(d.seasonal, rownames = T)
a.seasonal <- dcast(data = seasonal, formula = genome ~ season + year, value.var = "abund.perc")
a.seasonal <- as.matrix(a.seasonal, rownames = T)

d.annual <- dcast(data = annual, formula = genome ~ year, value.var = "nuc.div")
d.annual <- as.matrix(d.annual, rownames = T)
a.annual <- dcast(data = annual, formula = genome ~ year, value.var = "abund.perc")
...
for (g in cor.seasonal$genome1){
  ...
  p.seas <- p.seas.abund + p.seas.div + plot_layout(nrow = 2, guides = "collect") +
    ...
    pdf(file = file.path(output.plot.folder,"to-choose-cutoffs-take3/seasonal",paste(tax$full.taxonomy[tax$bin.full.name == g], tax$bin.full.name[tax$bin.full.name == g], tax$num.in.cluster[tax$bin.full.name == g], ".pdf")),
        width = 10, height = 9)
    print(p.seas)
    dev.off()
...
for (g in cor.annual$genome1){
  ...
  pdf(file = file.path(output.plot.folder,"to-choose-cutoffs-take3/annual",paste(tax$full.taxonomy[tax$bin.full.name == g], tax$bin.full.name[tax$bin.full.name == g], tax$num.in.cluster[tax$bin.full.name == g], ".pdf")),
      width = 10, height = 9)
  print(p.time)
  dev.off()
